...
void PressurePPLose(u8 target, u8 attacker, u16 move)
{
    ...
    if (GetBattlerAbility(target) != ABILITY_PRESSURE || !SpeciesHasInnate(gBattleMons[target].species, ABILITY_PRESSURE))
        return;
...
void PressurePPLoseOnUsingImprison(u8 attacker)
{
    ...
    {
        if (atkSide != GetBattlerSide(i) && (GetBattlerAbility(i) == ABILITY_PRESSURE || SpeciesHasInnate(gBattleMons[i].species, ABILITY_PRESSURE)))
        {
...
void PressurePPLoseOnUsingPerishSong(u8 attacker)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if ((GetBattlerAbility(i) == ABILITY_PRESSURE || SpeciesHasInnate(gBattleMons[i].species, ABILITY_PRESSURE)) && i != attacker)
        {
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_OCTOLOCK:
            if (gDisableStructs[gActiveBattler].octolock 
             && !(GetBattlerAbility(gActiveBattler)    == ABILITY_CLEAR_BODY       || SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_CLEAR_BODY)
                  || GetBattlerAbility(gActiveBattler) == ABILITY_FULL_METAL_BODY  || SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_FULL_METAL_BODY)
                  || GetBattlerAbility(gActiveBattler) == ABILITY_WHITE_SMOKE      || SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_WHITE_SMOKE)))
            {
                gBattlerTarget = gActiveBattler;
            ...
            if (gBattleMons[gActiveBattler].status2 & STATUS2_UPROAR)
            {
                for (gBattlerAttacker = 0; gBattlerAttacker < gBattlersCount; gBattlerAttacker++)
                {
                    if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
				     ...
				     && !SpeciesHasInnate(gBattleMons[gBattlerAttacker].species, ABILITY_SOUNDPROOF))
                    {
    ...
    do
    {
        ...
        case CANCELLER_MULTIHIT_MOVES:
        {
            if (gBattleMoves[gCurrentMove].effect == EFFECT_MULTI_HIT)
            {
                ...
                if (ability == ABILITY_SKILL_LINK || SpeciesHasInnate(gBattleMons[gBattlerAttacker].species, ABILITY_SKILL_LINK))
                {
                    gMultiHitCounter = 5;
                }
...
bool32 TryChangeBattleWeather(u8 battler, u32 weatherEnumId, bool32 viaAbility)
{
    if (gBattleWeather & WEATHER_PRIMAL_ANY
		...
		&& !SpeciesHasInnate(gBattleMons[battler].species, ABILITY_DESOLATE_LAND)
        && GetBattlerAbility(battler) != ABILITY_PRIMORDIAL_SEA
		&& !SpeciesHasInnate(gBattleMons[battler].species, ABILITY_PRIMORDIAL_SEA)
        && (GetBattlerAbility(battler) != ABILITY_DELTA_STREAM
		&& !SpeciesHasInnate(gBattleMons[battler].species, ABILITY_DELTA_STREAM)))
    {
        return FALSE;
    }
...
bool32 ShouldChangeFormHpBased(u32 battler)
{
    ...
    static const u16 forms[][4] =
    {
        ...
        {ABILITY_ZEN_MODE, SPECIES_DARMANITAN_GALARIAN, SPECIES_DARMANITAN_ZEN_MODE_GALARIAN, 1},
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u16 ability, u8 special, u16 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
		...
		*/
		...
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_DRIZZLE)){
            ...
            else if (gBattleWeather & B_WEATHER_PRIMAL_ANY && WEATHER_HAS_EFFECT && !gSpecialStatuses[battler].switchInInnateDone[GetSpeciesInnateNum(gBattleMons[battler].species, ABILITY_DRIZZLE)])
            {
		...
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_SAND_STREAM)){
            ...
            else if (gBattleWeather & B_WEATHER_PRIMAL_ANY && WEATHER_HAS_EFFECT && !gSpecialStatuses[battler].switchInInnateDone[GetSpeciesInnateNum(gBattleMons[battler].species, ABILITY_SAND_STREAM)])
            {
		...
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_DROUGHT)){
            ...
            else if (gBattleWeather & B_WEATHER_PRIMAL_ANY && WEATHER_HAS_EFFECT && !gSpecialStatuses[battler].switchInInnateDone[GetSpeciesInnateNum(gBattleMons[battler].species, ABILITY_DROUGHT)])
            {
		...
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_SNOW_WARNING)){
            ...
            else if (gBattleWeather & B_WEATHER_PRIMAL_ANY && WEATHER_HAS_EFFECT && !gSpecialStatuses[battler].switchInInnateDone[GetSpeciesInnateNum(gBattleMons[battler].species, ABILITY_SNOW_WARNING)])
            {
		...
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_DOWNLOAD)){
            if (!gSpecialStatuses[battler].switchInInnateDone[GetSpeciesInnateNum(gBattleMons[battler].species, ABILITY_DOWNLOAD)])
            {
                u32 statId, opposingBattler;
                u32 opposingDef = 0, opposingSpDef = 0;
                gBattleScripting.abilityPopupOverwrite = ABILITY_DOWNLOAD;
		...
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_SUN_WORSHIP)){
			...
			&& IsBattlerWeatherAffected(battler, WEATHER_SUN_ANY))
            {
				...
				u32 userSpDef = gBattleMons[battler].spDefense * gStatStageRatios[gBattleMons[battler].statStages[STAT_SPDEF]][0] / gStatStageRatios[gBattleMons[battler].statStages[STAT_SPDEF]][1]; 
				u32 userSpd = gBattleMons[battler].speed * gStatStageRatios[gBattleMons[battler].statStages[STAT_SPEED]][0] / gStatStageRatios[gBattleMons[battler].statStages[STAT_SPEED]][1];
				gBattleScripting.abilityPopupOverwrite = ABILITY_SUN_WORSHIP;
				gLastUsedAbility = ABILITY_SUN_WORSHIP;
    ...
    case ABILITYEFFECT_ENDTURN: // 1
        if (gBattleMons[battler].hp != 0)
        {
			...
			// Harvest
			if(SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_HARVEST)){
                if ((IsBattlerWeatherAffected(battler, WEATHER_SUN_ANY) || Random() % 2 == 0)
                 && gBattleMons[battler].item == ITEM_NONE
                 && gBattleStruct->changedItems[battler] == ITEM_NONE   // Will not inherit an item
                 && ItemId_GetPocket(GetUsedHeldItem(battler)) == POCKET_BERRIES)
			...
			// Hydration
            if(SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_HYDRATION)){
                if (IsBattlerWeatherAffected(battler, WEATHER_RAIN_ANY)
                 && gBattleMons[battler].status1 & STATUS1_ANY)
                {
			...
			// Moody
            if(SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_MOODY)){
                if (gDisableStructs[battler].isFirstTurn != 2)
                {
                    u32 validToRaise = 0, validToLower = 0;
                    u32 statsNum = (B_MOODY_ACC_EVASION != GEN_8) ? NUM_BATTLE_STATS : NUM_STATS;
			...
			// Truant
            if(SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_TRUANT)){
                gDisableStructs[gBattlerAttacker].truantCounter ^= 1;
            }
			
			// Bad Dreams
            if(SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_BAD_DREAMS)){
                if (gBattleMons[battler].status1 & STATUS1_SLEEP
                    || gBattleMons[BATTLE_OPPOSITE(battler)].status1 & STATUS1_SLEEP
                    || GetBattlerAbility(battler) == ABILITY_COMATOSE
                    || GetBattlerAbility(BATTLE_OPPOSITE(battler)) == ABILITY_COMATOSE)
			...
			// Sweet Dreams
            if(SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_SWEET_DREAMS)){
                if (!BATTLER_MAX_HP(gActiveBattler) && !(gStatuses3[gActiveBattler] & STATUS3_HEAL_BLOCK) && gBattleMons[battler].status1 & STATUS1_SLEEP)
                {
                    gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / 8;
                    if (gBattleMoveDamage == 0)
            ...
            if(SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_ZEN_MODE) ||
				SpeciesHasInnate(gBattleMons[gActiveBattler].species, ABILITY_SHIELDS_DOWN)){
                if ((effect = ShouldChangeFormHpBased(battler)))
                    BattleScriptPushCursorAndCallback(BattleScript_AttackerFormChangeEnd3);
			}
    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        if ((gLastUsedAbility == ABILITY_SOUNDPROOF && gBattleMoves[move].flags & FLAG_SOUND && !(gBattleMoves[move].target & MOVE_TARGET_USER))
			|| (SpeciesHasInnate(gBattleMons[battler].species, ABILITY_SOUNDPROOF) && gBattleMoves[move].flags & FLAG_SOUND && !(gBattleMoves[move].target & MOVE_TARGET_USER))
            || (gLastUsedAbility == ABILITY_BULLETPROOF && gBattleMoves[move].flags & FLAG_BALLISTIC)
			|| (SpeciesHasInnate(gBattleMons[battler].species, ABILITY_BULLETPROOF) && gBattleMoves[move].flags & FLAG_BALLISTIC))
        {
                 ...
                 && GetBattlerSide(gBattlerAttacker) != GetBattlerSide(battler))
        {
          ...
          && !(IS_MOVE_STATUS(move) && (GetBattlerAbility(gBattlerTarget) == ABILITY_MAGIC_BOUNCE || SpeciesHasInnate(gBattleMons[gBattlerTarget].species, ABILITY_MAGIC_BOUNCE))))
        {
    ...
    case ABILITYEFFECT_ABSORBING: // 3
        if (move != MOVE_NONE)
        {
            ...
            // Sap Sipper
			if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_SAP_SIPPER)){
				if (moveType == TYPE_GRASS){
					u16 userAttack = 0;					
					u16 userSpAttack = 0;
                    effect = 2;
            ...
            // Ice Dew
			if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_ICE_DEW)){
				if (moveType == TYPE_ICE){
					u16 userAttack = 0;					
					u16 userSpAttack = 0;
                    effect = 2;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_TANGLING_HAIR:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && (CompareStat(gBattlerAttacker, STAT_SPEED, MIN_STAT_STAGE, CMP_GREATER_THAN) || 
				 ...
				 SpeciesHasInnate(gBattleMons[gBattlerAttacker].species, ABILITY_MIRROR_ARMOR))
		...
		// Flame Body (Defender)
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_FLAME_BODY)){
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && gBattleMons[gBattlerAttacker].hp != 0
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && TARGET_TURN_DAMAGED
		...
		//Weak Armor
		if(SpeciesHasInnate(gBattleMons[battler].species, ABILITY_WEAK_ARMOR)){
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && TARGET_TURN_DAMAGED
             && IsBattlerAlive(battler)
             && IS_MOVE_PHYSICAL(gCurrentMove)
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
		...
		//Electric Burst
		if (SpeciesHasInnate(gBattleMons[battler].species, ABILITY_ELECTRIC_BURST)){
			if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && TARGET_TURN_DAMAGED // Need to actually hit the target
			 && gBattleMoves[move].type == TYPE_ELECTRIC) //Electric Type Moves
		...
		//Solenoglyphs
		if (SpeciesHasInnate(gBattleMons[battler].species, ABILITY_SOLENOGLYPHS)){
			if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && gBattleMons[gBattlerTarget].hp != 0
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && CanBePoisoned(gBattlerAttacker, gBattlerTarget)
		...
		// Poison Touch
		if (SpeciesHasInnate(gBattleMons[battler].species, ABILITY_POISON_TOUCH)){
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             && gBattleMons[gBattlerTarget].hp != 0
             && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
             && CanBePoisoned(gBattlerAttacker, gBattlerTarget)
...
u32 GetBattlerAbility(u8 battlerId)
{
    ...
    if ((((gBattleMons[gBattlerAttacker].ability == ABILITY_MOLD_BREAKER
	        || SpeciesHasInnate(gBattleMons[gBattlerAttacker].species, ABILITY_MOLD_BREAKER)
...
u32 IsAbilityOnSide(u32 battlerId, u32 ability)
{
    if (IsBattlerAlive(battlerId) && GetBattlerAbility(battlerId) == ability)
        return battlerId + 1;
	else if (IsBattlerAlive(battlerId) && SpeciesHasInnate(gBattleMons[battlerId].species, ability))
        return battlerId + 1;
    else if (IsBattlerAlive(BATTLE_PARTNER(battlerId)) && GetBattlerAbility(BATTLE_PARTNER(battlerId)) == ability)
        return BATTLE_PARTNER(battlerId) + 1;
	else if (IsBattlerAlive(BATTLE_PARTNER(battlerId)) && SpeciesHasInnate(gBattleMons[BATTLE_PARTNER(battlerId)].species, ability))
        return BATTLE_PARTNER(battlerId) + 1;
    else
        return 0;
}
...
u32 IsAbilityOnField(u32 ability)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (IsBattlerAlive(i) && (GetBattlerAbility(i) == ability || SpeciesHasInnate(gBattleMons[i].species, ability)))
            return i + 1;
    }
...
u32 IsAbilityOnFieldExcept(u32 battlerId, u32 ability)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (i != battlerId && IsBattlerAlive(i) && (GetBattlerAbility(i) == ability || SpeciesHasInnate(gBattleMons[i].species, ability)))
            return i + 1;
    }
...
bool32 HasEnoughHpToEatBerry(u32 battlerId, u32 hpFraction, u32 itemId)
{
    ...
    if (hpFraction <= 4 && (GetBattlerAbility(battlerId) == ABILITY_GLUTTONY || SpeciesHasInnate(gBattleMons[battlerId].species, ABILITY_GLUTTONY)) && isBerry
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
	...
	// Target's Innates
	// Thick Fat
	if(SpeciesHasInnate(gBattleMons[battlerDef].species, ABILITY_THICK_FAT)){
        if (moveType == TYPE_FIRE || moveType == TYPE_ICE)
        {
            MulModifier(&modifier, UQ_4_12(0.5));
            if (updateFlags)
	...
	// Innates
	// Antarctic Bird
	if(SpeciesHasInnate(gBattleMons[battlerAtk].species, ABILITY_ANTARCTIC_BIRD)){
		if (moveType == TYPE_FLYING || moveType == TYPE_ICE)
        {
            MulModifier(&modifier, UQ_4_12(1.5));
        }
...
static u32 CalcFinalDmg(u32 dmg, u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, u16 typeEffectivenessModifier, bool32 isCrit, bool32 updateFlags)
{
    ...
    if (((gSideStatuses[defSide] & SIDE_STATUS_REFLECT && IS_MOVE_PHYSICAL(move))
		...
		&& !SpeciesHasInnate(gBattleMons[battlerAtk].species, ABILITY_INFILTRATOR)
        && !(isCrit)
        && !gProtectStructs[gBattlerAttacker].confusionSelfDmg)
    {
        if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
	...
	if(abilityAtk == ABILITY_PARENTAL_BOND || SpeciesHasInnate(gBattleMons[gBattlerAttacker].species, ABILITY_PARENTAL_BOND)){
...
static void MulByTypeEffectiveness(u16 *modifier, u16 move, u8 moveType, u8 battlerDef, u8 defType, u8 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_FIGHTING || moveType == TYPE_NORMAL) && defType == TYPE_GHOST && (GetBattlerAbility(battlerAtk) == ABILITY_SCRAPPY || SpeciesHasInnate(gBattleMons[battlerAtk].species, ABILITY_SCRAPPY)) && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
            RecordAbilityBattle(battlerAtk, ABILITY_SCRAPPY);
    }
	else if (moveType == TYPE_ELECTRIC && defType == TYPE_GROUND && (GetBattlerAbility(battlerAtk) == ABILITY_GROUND_SHOCK || SpeciesHasInnate(gBattleMons[battlerAtk].species, ABILITY_GROUND_SHOCK)) && mod == UQ_4_12(0.0))
    {
	...
	else if (moveType == TYPE_ELECTRIC && defType == TYPE_ELECTRIC && (GetBattlerAbility(battlerAtk) == ABILITY_OVERCHARGE || SpeciesHasInnate(gBattleMons[battlerAtk].species, ABILITY_OVERCHARGE)) && mod == UQ_4_12(0.0))
    {
		//Has Innate Effect here too
...
static u16 CalcTypeEffectivenessMultiplierInternal(u16 move, u8 moveType, u8 battlerAtk, u8 battlerDef, bool32 recordAbilities, u16 modifier)
{
	...
	if ((GetBattlerAbility(battlerDef) == ABILITY_WEATHER_CONTROL || SpeciesHasInnate(gBattleMons[battlerDef].species, ABILITY_WEATHER_CONTROL)) && (gBattleMoves[move].flags & FLAG_WEATHER_BASED))
    {
        modifier = UQ_4_12(0.0);
        if (recordAbilities)
        {
...
u16 CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (abilityDef == ABILITY_WONDER_GUARD && modifier <= UQ_4_12(1.0) && gBattleMoves[move].power)
            modifier = UQ_4_12(0.0);
		if ((gBattleMoves[move].flags & FLAG_WEATHER_BASED) && (abilityDef == ABILITY_WEATHER_CONTROL || SpeciesHasInnate(speciesDef, ABILITY_WEATHER_CONTROL))) 
            modifier = UQ_4_12(0.0);
    }
