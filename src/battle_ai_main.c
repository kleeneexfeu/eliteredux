...
void GetAiLogicData(void)
{
    ...
    for (battlerAtk = 0; battlerAtk < MAX_BATTLERS_COUNT; battlerAtk++)
    {
        if (IsBattlerAlive(battlerAtk)) {
            ...
            AI_DATA->hpPercents[battlerAtk] = AI_DATA->hpPercents[battlerAtk];
            AI_DATA->moveLimitations[battlerAtk] = CheckMoveLimitations(battlerAtk, 0, 0);
...
static s16 AI_CheckBadMove(u8 battlerAtk, u8 battlerDef, u16 move, s16 score)
{
    ...
    if (!(gBattleMoves[move].target & MOVE_TARGET_USER))
    {
        ...
        if(gBattleMoves[move].flags & FLAG_SOUND && (AI_DATA->abilities[battlerAtk] == ABILITY_SAND_SONG || SpeciesHasInnate(gBattleMons[battlerAtk].species, ABILITY_SAND_SONG)))
        {
        ...
        if (moveType == TYPE_ROCK
          && (AI_DATA->abilities[battlerDef] == ABILITY_MOMENTUM || BattlerHasInnate(battlerDef, ABILITY_MOMENTUM)))
        {
            RETURN_SCORE_MINUS(20);
        }
    ...
    // other
        ...
        case EFFECT_WILL_O_WISP:
            if (!AI_CanBurn(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move, AI_DATA->partnerMove)
                 || (AI_DATA->abilities[battlerDef] == ABILITY_FLASH_FIRE))
                score -= 10;
            break;
        case EFFECT_MEMENTO:
        ...
        case EFFECT_TEETER_DANCE:
            if (((gBattleMons[battlerDef].status2 & STATUS2_CONFUSION)
             ...
             && ((gBattleMons[BATTLE_PARTNER(battlerDef)].status2 & STATUS2_CONFUSION)
              ...
              || (IsBattlerGrounded(BATTLE_PARTNER(battlerDef)) && (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN))
        ...
        case EFFECT_VITAL_THROW:
            if (IsAiFaster(AI_CHECK_FASTER) && AI_DATA->hpPercents[battlerAtk] < 40)
...
static s16 AI_CheckViability(u8 battlerAtk, u8 battlerDef, u16 move, s16 score)
{
    ...
    bool32 isDoubleBattle = IsValidDoubleBattle(battlerAtk);
    u32 i;
    u8 atkHpPercent = AI_DATA->hpPercents[battlerAtk];
